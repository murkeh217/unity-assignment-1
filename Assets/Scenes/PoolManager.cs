...
public static class PoolManager
{
    ...
    //gives number of items in the pool 
    public static int GetPoolCount(GameObject prefab)
    {
        if (pools.ContainsKey(prefab))
        {
            return pools[prefab].totalCount;
        }
        else
        {
            return 0;
    ...
    //gives active items in the pool 
    public static LinkedList<GameObject> GetActiveItems(GameObject prefab)
    {
        return pools[prefab].activeObjects;
    }

    //gives inactive items in the pool 
    public static LinkedList<GameObject> GetInactiveItems(GameObject prefab)
    {
        return pools[prefab].inactiveObjects;
    }
